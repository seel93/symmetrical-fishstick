...
class Agent:
    ...
    def __init__(
        ...
        self.model = Network(n_inputs, n_outputs).to("cpu")
    ...
    def update(self) -> None:
        ...
        batched_state = torch.tensor(
            np.array([b[0] for b in batch]), dtype=torch.float, device="cpu"
        )

        batched_action = torch.tensor(
            np.array([b[1] for b in batch]), dtype=torch.long, device="cpu"
        )
        batched_reward = torch.tensor(
            np.array([b[2] for b in batch]), dtype=torch.float, device="cpu"
        )
        batched_next_state = torch.tensor(
            np.array([b[3] for b in batch]), dtype=torch.float, device="cpu"
        )
        batched_next_action = torch.tensor(
            np.array([b[4] for b in batch]), dtype=torch.long, device="cpu"
        )

        batched_dones = torch.tensor(
            np.array([b[5] for b in batch]), dtype=torch.float, device="cpu"
        )
